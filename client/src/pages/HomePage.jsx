                    ...
                    <>
                        {trending[index] && (
                            <div className="relative w-full h-full overflow-hidden md:-mt-24 -mt-4">
                                ...
                                <div className="absolute md:top-[45vh] top-[65vh] left-4 right-4 md:left-28 md:w-1/3 text-white p-4 md:p-6 bg-zinc-900 bg-opacity-50 rounded-lg shadow-xl">
                                    <h1 className="text-start text-xl md:text-5xl font-extrabold text-green-400">{trending[index]?.title || "Titre non disponible"}</h1>
                                    <div className="flex flex-col">
                                        <p className="text-start text-sm md:text-lg">{trending[index]?.release_date || "Date de sortie inconnue"}</p>
                                        <div className="flex justify-between">
                                            <div className="flex items-center space-x-2">
                                                <FaStar size={20} color="yellow" className="md:text-lg" />
                                                <p className="text-sm md:text-lg"> {trending[index]?.vote_average ? Math.round(trending[index]?.vote_average * 100) / 100 : "-"} / 10</p>
                                            </div>
                                            <button
                                    ...
                                    >
                                        ...
                                        <div className="absolute inset-0 bg-black bg-opacity-50 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-4">
                                            ...
                                            <div className="flex items-center space-x-2 mt-2">
                                                ...
                                                <p className="text-sm md:text-lg text-white">{Math.round(data.vote_average * 100) / 100} / 10</p>
                                            </div>
                                        </div>
                                    </div>
                                ))}
